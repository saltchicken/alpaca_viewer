...
def main():
    ...
    if not api_key or not api_secret:
        print("Error: ALPACA_API_KEY or ALPACA_API_SECRET not found in .env file.", file=sys.stderr)
        sys.exit(1)

    parser = argparse.ArgumentParser(description="Fetch daily OHLCV data from Alpaca for database tickers.")
    parser.add_argument("--db-url", required=True, type=str, help="PostgreSQL Connection URL")
    parser.add_argument("--days-back", type=int, default=1, help="Days back to fetch (1 = yesterday/most recent)")
    ...
    start_dt = end_dt - timedelta(days=args.days_back + 3) # Add buffer for weekends
    ...
    for i in range(0, len(tickers), chunk_size):
        ...
        print(f" -> Fetching Alpaca data for batch {i + 1} to {min(i + chunk_size, len(tickers))}...")
    ...
    all_bars_df['ticker'] = all_bars_df['ticker'].str.replace(".", "-")
    ...
    print(f"Saving {len(all_bars_df)} price records for {latest_date} to table '{db_table}'...")
